...
//#region node_modules/grpc-web/index.js
var require_grpc_web = /* @__PURE__ */ __commonJS({ "node_modules/grpc-web/index.js": ((exports, module) => {
	var n, p = p || {}, q = exports || self;
	function r(a, b) {
		a = a.split(".");
		b = b || q;
	...
	var u = "closure_uid_" + (Math.random() * 1e9 >>> 0), ba = 0;
	function ca(a, b, c) {
		...
		for (b instanceof Uint8Array || b instanceof Array ? h = b : h = new Uint8Array(b); k < h.length;) {
			switch (f.h) {
				...
				case ta:
					e(h[k]);
					break;
				default: throw Error("unexpected parser state: " + f.h);
			}
			f.v++;
			k++;
		}
	...
	var ua = 0, sa = 1, ta = 2, va = 3, qa = 0, B = 128;
	function ra(a, b, c, d) {
		a.h = va;
	...
	var C = class extends Error {
		...
		toString() {
			let a = `RpcError(${ya(this.code) || String(this.code)})`;
	...
	Ba[" "] = function() {};
	var Ca = D("Trident") || D("MSIE"), Da = D("Gecko") && !(Aa().toLowerCase().indexOf("webkit") != -1 && !D("Edge")) && !(D("Trident") || D("MSIE")) && !D("Edge");
	var Fa = class {
		constructor(a) {
	...
	function La(a, b) {
		...
		for (let e = 1; e < arguments.length; e++) {
			d = arguments[e];
			for (c in d) a[c] = d[c];
			for (let g = 0; g < Ka.length; g++) c = Ka[g], Object.prototype.hasOwnProperty.call(d, c) && (a[c] = d[c]);
		}
	}
	var Ma = {};
	var Na = class {
	...
	var $a = "closure_lm_" + (Math.random() * 1e6 | 0), ab = {}, bb = 0;
	function cb(a, b, c, d, e) {
		if (d && d.once) db(a, b, c, d, e);
		else if (Array.isArray(b)) for (var g = 0; g < b.length; g++) cb(a, b[g], c, d, e);
		else c = eb(c), a && a[H] ? a.j.add(String(b), c, !1, t(d) ? !!d.capture : !!d, e) : fb(a, b, c, !1, d, e);
	}
	function fb(a, b, c, d, e, g) {
		if (!b) throw Error("Invalid event type");
		var f = t(e) ? !!e.capture : !!e, h = gb(a);
		h || (a[$a] = h = new J(a));
		c = h.add(b, c, d, f, g);
		if (!c.proxy) {
			d = hb();
			...
			d.src = a;
			d.listener = c;
			if (a.addEventListener) Ua || (e = f), e === void 0 && (e = !1), a.addEventListener(b.toString(), d, e);
	...
	function db(a, b, c, d, e) {
		if (Array.isArray(b)) for (var g = 0; g < b.length; g++) db(a, b[g], c, d, e);
		else c = eb(c), a && a[H] ? a.j.add(String(b), c, !0, t(d) ? !!d.capture : !!d, e) : fb(a, b, c, !0, d, e);
	}
	function ob(a, b, c, d, e) {
		if (Array.isArray(b)) for (var g = 0; g < b.length; g++) ob(a, b[g], c, d, e);
		else (d = t(d) ? !!d.capture : !!d, c = eb(c), a && a[H]) ? (a = a.j, b = String(b).toString(), b in a.g && (g = a.g[b], c = Ya(g, c, d, e), c > -1 && (I(g[c]), Array.prototype.splice.call(g, c, 1), g.length == 0 && (delete a.g[b], a.h--)))) : a && (a = gb(a)) && (b = a.g[b.toString()], a = -1, b && (a = Ya(b, c, d, e)), (c = a > -1 ? b[a] : null) && pb(c));
	}
	function pb(a) {
	...
	function L(a, b) {
		...
		a = a.W;
		d = b.type || b;
		if (typeof b === "string") b = new F(b, a);
		else if (b instanceof F) b.target = b.target || a;
		else {
			var e = b;
			b = new F(d, a);
			La(b, e);
	...
	}, O = new N("OFF", Infinity), tb = new N("SEVERE", 1e3), ub = new N("CONFIG", 700), vb = new N("FINE", 500), wb = class {}, xb, yb = class {
		constructor(a, b, c) {
	...
	function Fb(a, b, c) {
		...
		d && (b = b || O, d = P(Eb(), a.getName()), typeof c === "function" && (c = c()), xb ||= new wb(), a = a.getName(), a = new yb(b, c, a), Ab(d, a));
	}
	function Gb(a, b) {
		a && Fb(a, tb, b);
	}
	...
	function Rb(a, b, c) {
		...
		b = c || "";
		c = new Map(a.headers);
		const d = Array.from(c.keys()).find((g) => "content-type" == g.toLowerCase()), e = q.FormData && b instanceof q.FormData;
		!(za(Qb, "POST") >= 0) || d || e || c.set("Content-Type", "application/x-www-form-urlencoded;charset=utf-8");
		for (const [g, f] of c) a.g.setRequestHeader(g, f);
	...
	function Wb(a) {
		if (a.i && typeof p != "undefined") {
			if (a.J[1] && U(a) == 4 && a.getStatus() == 2) Q(a.h, S(a, "Local request error detected and ignored"));
			else if (a.F && U(a) == 4) sb(a.U, 0, a);
			else if (L(a, "readystatechange"), U(a) == 4) {
	...
	function W(a, b) {
		if (b.code != 0) {
			...
			for (let d = 0; d < a.j.length; d++) a.j[d](c);
		}
		b = {
			code: b.code,
	...
	function dc(a, b) {
		for (let c = 0; c < a.l.length; c++) a.l[c](b);
	}
	function ec(a) {
		for (let b = 0; b < a.i.length; b++) a.i[b]();
	}
	function X(a, b) {
		b = a.indexOf(b);
		b > -1 && a.splice(b, 1);
			...
			cb(this.g, "readystatechange", function() {
				...
				if (c = c.g ? c.g.getResponseHeader("Content-Type") : null) {
					...
					if (c.lastIndexOf("application/grpc-web-text", 0) == 0) {
						...
						b.o = d;
						c = $b(c);
					} else if (c.lastIndexOf("application/grpc", 0) == 0) c = new Uint8Array(Xb(b.g));
					else {
						...
						if (qa in d[c]) {
							...
							if (e) {
								...
								try {
									...
									W(b, new C(13, `Error when deserializing response data; error: ${k}, response: ${h}`));
								}
								if (g) {
									e = b;
									g = h;
	...
	function jc(a, b, c) {
		var d = b.getMethodDescriptor(), e = c + d.getName();
		c = a.i ? a.i : new R();
		c.O = a.j;
		const g = new Y({ $: c });
		g.v = d.h;
		...
		if (a.l) {
			...
			if (!l) if (k = fc(k), typeof e === "string") {
				if (l = encodeURIComponent("$httpHeaders"), k = k != null ? "=" + encodeURIComponent(String(k)) : "", l += k) k = e.indexOf("#"), k < 0 && (k = e.length), h = e.indexOf("?"), h < 0 || h > k ? (h = k, f = "") : f = e.substring(h + 1, k), e = [
					e.slice(0, h),
					f,
					e.slice(k)
				], k = e[1], e[1] = l ? k ? k + "&" + l : l : k, e = e[0] + (e[1] ? "?" + e[1] : "") + e[2];
		...
		if (a.g == "text") {
			...
			f = d;
			switch (a.length - k) {
				case 2: h = a[k + 1], f = m[(h & 15) << 2] || d;
				case 1: a = a[k], b[l] = m[a >> 2] + m[(a & 3) << 4 | h >> 4] + f + d;
			}
			b = b.join("");
	...
	function kc(a, b, c, d, e, g = {}) {
		const f = b.substr(0, b.length - e.name.length), h = g && g.signal;
		...
		}), a.m).call(a, ia(e, c, d)).then((k) => k.getResponseMessage());
	}
	var Z = class {
		constructor(a = {}, b) {
		...
		Y(a, b, c, d, e) {
			const g = a.substr(0, a.length - d.name.length);
			a = hc((f) => jc(this, f, g), this.h).call(this, ia(d, b, c));
			ic(a, e, !1);
			return new la(a);
		}
		unaryCall(a, b, c, d, e = {}) {
		...
		Z(a, b, c, d) {
			...
			return hc((g) => jc(this, g, e), this.h).call(this, ia(d, b, c));
		}
	};
...
//#endregion
export default require_grpc_web();
