...
func (s *ComputeServer) startWorker(id int, jobChan <-chan *Job, db *sql.DB) {
	...
	var q []*Job
	// NOTE: Đây là phiên bản PURE FIFO để đối chứng.
	// Không có logic Adaptive Switching, luôn xử lý người đến trước.
	...
	DrainLoop:
		...
		var job *Job
		// PURE FIFO: Luôn lấy việc ở ĐẦU hàng (Cũ nhất)
		job = q[0]
		q = q[1:] // Cắt đầu
		...
		// 1. Kiểm tra khách có hủy kèo chưa (Context Done) (Stale Job Dropping)
		select {
		case <-job.Ctx.Done():
			// Client đã huỷ hoặc Timeout -> Drop job này luôn để tiết kiệm CPU
			// Đây là cơ chế "Stale Job Dropping" bắt buộc để so sánh công bằng.
			// Trong trường hợp Overload, FIFO sẽ drop gần hết jobs ở đây vì chúng đều đã cũ (time-to-process > timeout).
			s.send(job, nil, job.Ctx.Err()) // Báo lỗi về cho caller (để tính vào Error Rate)
			continue
		default:
		}
...
func main() {
	...
	// FIFO: Sẽ lấy request thứ 1, 2, ..., 100.
	// Đến request thứ 5, thời gian chờ > 200ms -> Client đã Timeout -> Error.
	// =========================================================
	...
	// Sau cơn bão, tải giảm xuống.
	// Adaptive: Sẽ dọn dẹp hàng đợi nhanh để về trạng thái bình thường.
	// FIFO: Sẽ mất nhiều thời gian để xử lý hết đống rác cũ.
	// =========================================================
	// (Kịch bản 4 khó mô phỏng tách biệt trong code runLoadTest đơn giản này,
	// nhưng nó được phản ánh qua Time Taken của Kịch bản 3)
}
